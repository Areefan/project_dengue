...
rain['rain'] = rain['rain'].replace(-999.0, np.nan)
...
# %%
# print('Series has {} missing values'.format(rain.isna().sum()))
date_missed = rain[rain.isna()].index

# %%
# Group by the relevant column(s), ensure that 'date' is part of the index

rain.set_index(['source_file', 'date'], inplace=True)

# %%
# Resetting just the date index to work with it directly
df_resampled = rain.groupby(level='source_file').apply(
...
# %%
ST9601 =  newdf.groupby('date')["rain"].mean().reset_index()
...
with sa.create_engine(conn_str).connect() as con:
  newdf3.to_sql("rain",con,index=None, if_exists='replace')

# %%
print('uploaded rain success...')

# %%
newdf3.to_csv(r"./data/dataset/rain_all.csv", index=False)
